...
async def verify_database_state(database_url: str):
    ...
    try:
        ...
        if policy_table_exists:
            ...
            has_ohlc_policy = any(p['id'] == 'priceEodOHLC_dateRange' for p in policies)
            if has_ohlc_policy:
                print()
        ...
        print("❌ DB 인증 실패: 비밀번호가 올바르지 않습니다.")
    except asyncpg.exceptions.PostgresConnectionError as e:
        print(f"❌ DB 연결 실패: {e}")
        print("   Supabase 프로젝트가 일시중지 상태일 수 있습니다.")
        print("   https://supabase.com/dashboard 에서 프로젝트를 재개하세요.")
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("사용법: python verify_checklist_direct.py <DATABASE_URL>")
        print()
        print("DATABASE_URL은 다음 형식이어야 합니다:")
        print("postgresql://user:password@host:port/database")
        print()
        print("Supabase를 사용하는 경우:")
        print("1. https://supabase.com/dashboard 접속")
        print("2. 프로젝트 선택 → Settings → Database")
        print("3. Connection string (Pooler 모드) 복사")
        sys.exit(1)
